...
def init(self, theme_color=(31, 78, 121)):
super().init()
...
plt.style.use('ggplot')
fig, ax = plt.subplots(figsize=(5, 4))
ax.bar(['Volume', 'Persone'], [p_vol, p_pers], color=['#28a745', '#17a2b8'])
buf = io.BytesIO()
plt.savefig(buf, format='png', bbox_inches='tight')
buf.seek(0)
plt.close(fig)
...
pdf_bytes = pdf.output() # fpdf2 restituisce già i bytes
os.remove("t.png")
